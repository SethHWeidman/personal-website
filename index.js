...
app.get('/visitor-log', async (req, res) => {
    try {
      ...
      const result = await client.query('SELECT name, signed_at FROM visitor_log ORDER BY signed_at DESC');
...
function generateVisitorsTable(visitors) {
    ...
    visitors.forEach(visitor => {
      const formattedDate = new Date(visitor.signed_at).toLocaleDateString('en-US', {
        year: 'numeric', 
        month: 'long', 
        day: 'numeric'
      });
...
app.post('/sign-log', async (req, res) => {
    ...
    try {
      const client = await pool.connect();
      await client.query('INSERT INTO visitor_log (name, signed_at) VALUES ($1, $2)', [name, timestamp]);
      client.release();
      res.send(`<h1>Thanks, ${name}! You've signed the visitor log at ${timestamp}.</h1>`);
    } catch (err) {
      console.error(err);
